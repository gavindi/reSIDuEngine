...
float SID::processSID()
{
    ...
    if (externalInput != 0)
    {
        ...
        // Note: externalInput is persistent — it stays until changed by input() or reset().
        // This matches real hardware where EXT IN is a continuous signal (e.g. digiboost DC bias).
    }
 ...
 * the SID's own output or routed through the filter. This method sets a
 * persistent DC bias or audio signal that is mixed into every audio sample
 * until changed by another call to input() or a reset().
 *
 * Primary use: MOS8580 digiboost. Call input(-32768) after initialising the
 * chip to inject a full-scale negative DC bias into EXT IN. Rapid D418
 * volume-register writes then amplitude-modulate this bias, producing
 * audible digi samples through the external 1.6 Hz highpass filter.
 * For MOS6581, call input(0) (no digiboost needed — DAC leakage supplies
 * the bias naturally).
 *
 * The external input is routed to the filter when the EXTIN bit (bit 3) of
 * register 0xD417 is set; otherwise it is mixed directly with the output.
 *
 * @param value 16-bit signed bias to apply (-32768 to +32767)
 */

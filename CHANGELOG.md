...
### Fixed
- `sidplayer2`: NMI handlers are now transparent to a play routine they interrupt mid-execution, fixing silent playback of Rob Hubbard's `I, Ball` (subtune 1). Its IRQ music (at `$E009`, copied there from `$9000` by the init routine) is interrupted by a 7037 Hz CIA2 Timer A digi NMI. This player's 6502 core stores the program counter on the stack in a non-standard byte order (`[PCH][PCL]`, low byte pushed first — see the `JSR`/`RTS`/`RTI` handlers), so the injected NMI frame's `RTI` returned a byte-swapped PC (e.g. `$E01A` → `$1AE0`); when the NMI fired while the music was still running (`finished == 0`) that garbage PC crashed the player, leaked 8 stack bytes per NMI, and hit the instruction-timeout within ~30 firings. `executeNMI()` now restores the interrupted `PC`/`SP`/`ST` after the handler completes whenever the play routine was mid-execution (or the handler timed out), so the NMI only leaves its intended side effects (`$D400–$D41F` and CIA writes, sample-pointer advance). The between-frames case (`finished == 1`) keeps the previous behaviour that other digi tunes depend on
- `sidplayer2`: A one-shot digi NMI that disarms itself now actually stops. `I, Ball`'s handler retires the NMI via `$C03E` (`$DD0D = $7F`, clearing the CIA2 ICR enable mask) once its sample finishes; the player previously ignored this and kept firing the exhausted handler, spewing garbage `$D418` writes that collapsed playback to near-silence. A `cia2IcrMask` now tracks the CIA ICR with correct set/clear semantics (bit 7 selects set-or-clear of the 1-bits, so `$7F` clears all five sources although bit 0 reads as 1), and the audio callback only fires the NMI while Timer A is armed. The mask is keyed **only** on the ICR (`$DD0D`), not the timer control register (`$DD0E`): stopping the timer alone is a transient action other digi players perform every sample block and immediately re-arm (e.g. `BMX Kidz`), so honouring it there would wrongly silence them. Re-arming the ICR re-adopts the `$DD04/05` latch as the NMI period
- `reSIDuEngine`: Restored the additive master-volume DC bias in `processSID()` (`result = (output/0x10000) * volNorm + volNorm`). Volume-register (`$D418`) digi tunes play samples by writing the 4-bit master volume at a high rate; with silent voices the multiplied term alone is zero, so the digi was inaudible. The additive bias turns each volume write into an output step that the external 1.6 Hz high-pass converts into the audible digi waveform, while leaving ordinary steady-volume tunes unchanged. `BMX Kidz` relies on this — its music is driven from a raster loop this player cannot fully track, so its audio is carried entirely by the CIA2 NMI digi
- Verified per-second output levels against `sidplayfp` for `I, Ball`, `BMX Kidz`, `Skate or Die` (NMI digi), `Arkanoid` (secondary IRQ), `Commando`, `Delta`, `Monty on the Run`, `Sanxion`, `Comic Bakery`, `Times of Lore`, `Wizball`, and `Rambo` — all play
